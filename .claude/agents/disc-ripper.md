...
**This agent ONLY works with the `ripdisc` repository.**

- **Correct repo path:** The root of the `ripdisc` repository
- **Correct GitHub repo:** `stephenbeale/ripdisc`

Before making ANY git operations (commits, branches, PRs), ALWAYS verify you are in the correct repository:
```bash
git remote -v
# Must show: origin https://github.com/stephenbeale/ripdisc.git
```

**NEVER commit disc-ripping changes to:**
- `snout` repo (that's for eBay API)
- `claude` directory (that's also snout)
- Any other repository

If the current directory is not the ripdisc repo, **STOP** and ask the user to navigate to the correct repo first.

## Script Location
The script is located at: `.\rip-disc.ps1`
...
- Temporary MakeMKV output: `temp\<title>\` (Disc 1) or `temp\<title>\Disc<N>\` (Disc 2+)

## Command Templates

### Single Movie - D: internal (DriveIndex 0)
```powershell
.\rip-disc.ps1 -title "Movie Name" -DriveIndex 0
```

### Single Movie - G: ASUS external (DriveIndex 1)
```powershell
.\rip-disc.ps1 -title "Movie Name" -DriveIndex 1
```

### Single Movie - Output to F: drive
```powershell
.\rip-disc.ps1 -title "Movie Name" -DriveIndex 0 -OutputDrive "F:"
```

### TV Series (no season folder)
```powershell
.\rip-disc.ps1 -title "Series Name" -Series -DriveIndex 0
```

### TV Series (with season folder)
```powershell
.\rip-disc.ps1 -title "Series Name" -Series -Season 2 -DriveIndex 0
```

### TV Series Multi-Disc (continuing episodes)
```powershell
# Disc 1 (Episodes 1-4)
.\rip-disc.ps1 -title "Series Name" -Series -Season 1 -Disc 1 -DriveIndex 0

# Disc 2 (Episodes 5-8, continuing from episode 5)
.\rip-disc.ps1 -title "Series Name" -Series -Season 1 -Disc 2 -StartEpisode 5 -DriveIndex 0
```
...
.\rip-disc.ps1 -title "Movie Name" -Disc 2 -DriveIndex 1
```
...
1. **MakeMKV rip** - Rip disc to MKV files (to temp\<title>\ or temp\<title>\Disc<N>\ for disc 2+)
...
4. For multi-disc films:
   ...
   - Disc 1 uses: `temp\<title>\`, Disc 2+ uses: `temp\<title>\Disc<N>\`
...
## Important Notes
- **Window title**: The script sets the PowerShell window/tab title to the film name with status suffixes:
  - `The Dark Knight - INPUT` (waiting for user confirmation)
...
```powershell
.\rip-disc.ps1 -title "Inception" -DriveIndex 0
```
...
.\rip-disc.ps1 -title "The Dark Knight" -DriveIndex 0

# Disc 2 (Special Features) - run in parallel in Terminal 2 using external drive
.\rip-disc.ps1 -title "The Dark Knight" -Disc 2 -DriveIndex 1
```

You can run both discs **concurrently** using separate terminal windows and different drives:
- Temp files go to separate directories: `temp\The Dark Knight\Disc1\` and `temp\The Dark Knight\Disc2\`
...
.\rip-disc.ps1 -title "Breaking Bad" -Series -Season 3 -Disc 1 -DriveIndex 0

# Disc 2 (use -StartEpisode with the number shown after disc 1 completes)
.\rip-disc.ps1 -title "Breaking Bad" -Series -Season 3 -Disc 2 -StartEpisode 5 -DriveIndex 0
```
...
```powershell
.\rip-disc.ps1 -title "Roots" -Series -DriveIndex 0
```
...
|----------|--------|----------|
| `handbrake` | 2 | MKV files in `temp\{title}\` |
...
```powershell
.\continue-rip.ps1 -title "Breaking Bad" -Series -Season 2 -FromStep organize
```
