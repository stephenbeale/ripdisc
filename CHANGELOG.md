...
> **Superseded by PR #138 (same day).** The inline `.TrimEnd('.', ' ')` described above now lives inside the shared `Get-SafeTitle` function, and `tests/Test-TitleSanitization.ps1` **no longer exists** - it scraped the `$safeTitle = ...` expression text and evaluated it with `Invoke-Expression`, which cannot work once the logic is a function call. Its cases (including `"W."`, `"W.."` and the trailing-space and PR #131 cases) are all covered by `tests/Test-TitleAndDriveSanitization.ps1`, which AST-extracts the real function body instead. The fix itself is unchanged in behaviour - only where it lives. This entry is kept as-is because it is accurate history of what shipped at the time.
